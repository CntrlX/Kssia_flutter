...
class _DetailsPageState extends ConsumerState<DetailsPage> {
  ...
  void _onAwardEdit(int index) async {
    ...
    awardAuthorityController.text = oldAward.authorityName ?? '';
  ...
  Future<void> _addNewBrochure() async {
    ...
    final String brochureUrl = await api.createFileUrl(
      _brochurePdfFile!.path,
    );
  ...
  void _editWebsite(int index) {
    ...
    websiteLinkController.text =
        ref.read(userProvider).value?.websites?[index].url ?? '';

    showWebsiteSheet(
        addWebsite: () {
          final Website oldWebsite =
              ref.read(userProvider).value!.websites![index];
          final Website newWebsite = Website(
              name: websiteNameController.text,
              url: websiteLinkController.text);
  ...
  void _editVideo(int index) {
    videoNameController.text =
        ref.read(userProvider).value?.video?[index].name ?? '';
    videoLinkController.text =
        ref.read(userProvider).value?.video?[index].url ?? '';

    showVideoLinkSheet(
        addVideo: () {
          final Video oldVideo = ref.read(userProvider).value!.video![index];
          final Video newVideo = Video(
              name: videoNameController.text, url: videoLinkController.text);
